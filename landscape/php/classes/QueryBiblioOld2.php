    ...
    private function getNewResults(stdClass $params)
    {
		...
		$limit = ($params->limit == '') ? 100: $params->limit;
                ...
                if (isset($params->abstr)){ 
					...
					$queryText='with abstract_filter("ART_ID") as(SELECT distinct a."ART_ID" FROM landscape."ABSTRACT" as a where  to_tsvector(\'english\',s."TEXT") @@ to_tsquery(\'english\',\''.$abstrText.'\'))';
					...
					/*$queryText=$queryText.',abstract_filter("ART_ID") as(                     
                                                ...
                                                where  to_tsvector(\'english\',s."TEXT") @@ to_tsquery(\'english\',\''.$abstrText.'\')) as a 
                                                order by a."ART_ID")';*/
                ...
                if (isset($params->spatial)){
                   ...
                   $queryText=$queryText.',spatial_filter("ART_ID") as(
                                    select  distinct * from get_extent_bboxes('.$top.','.$left.','.$bottom.','.$right.') as art inner join abstract_filter as lf on art."ART_ID"=lf.ART_ID)';
					}
                ...
                //get aspatial params
                 if (isset($params->l3value)){                  
                    ...
                    $queryText=$queryText.',level_filter("ART_ID") as(
                                            ...
                                            inner join '.$filter.' as a on al."ART_ID"=a."ART_ID" 
                ...
                if (isset($params->title)){ 
                    ...
                    if($type=='or'){
                     ...
                     array_push($arrayAttr,'SELECT art."ART_ID"
                                                FROM landscape."ARTICLE" as art inner join  '.$filter.' as lf on art."ART_ID"=lf."ART_ID" 
                                                WHERE to_tsvector(\'english\',art."TITLE") @@ to_tsquery(\'english\',\''.$titleText.'\')');   
                        
                    }
                    if($type=='and'){ 
                        //array_push($arrayAttr,'art."TITLE"~* \''.$title.'\'');
                        array_push($arrayAttr,'to_tsvector(\'english\',art."TITLE") @@ to_tsquery(\'english\',\''.$titleText.'\')');
